 ...
 */
import { Parameter, randomId } from "@turbo-agent/copilotkit-shared";
...
export function useCopilotAction<const T extends Parameter[] | [] = []>(
  ...
  if (
    ...
    action.render = ((props: ActionRenderProps<T> & { messageId?: string }): string => {
      ...
      const isNoArgsRenderWait = (
        _fn:
          | ((props: ActionRenderPropsNoArgsWait<T>) => string)
          | ((props: ActionRenderPropsWait<T>) => string),
      ): _fn is (props: ActionRenderPropsNoArgsWait<T>) => string => {
      ...
      // Return empty string instead of Fragment
      return "";
    }) as any;
  }
