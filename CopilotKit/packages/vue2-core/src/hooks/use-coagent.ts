...
export function useCoAgent<T = any>(
  ...
  if (!contextVueInstance) {
    // In Vue2, we can't easily get current instance outside of component context
    console.warn('useCoAgent: Vue instance not provided, using fallback implementation');
  ...
  const reactiveCoAgent = Vue.observable({
    name,
    nodeName: undefined as string | undefined,
    threadId: undefined as string | undefined,
    running: false,
    state: (isInternalStateManagementWithInitial(options) ? options.initialState : {}) as T,
    setState: null as any, // Will be set after setState function is defined
    start,
    stop,
    run,
  });

  // State setter function with enhanced logging
  const setState = (newState: T | ((prevState: T | undefined) => T)) => {
    console.log('CoAgent setState called:', { name, newState });
    ...
    console.log('Current coagent state:', coagentState);
    ...
    console.log('Setting new coagent states:', newCoagentStates);

    context.setCoagentStatesWithRef(newCoagentStates);
    
    // Force update the reactive object immediately using Vue.set for deep reactivity
    Vue.set(reactiveCoAgent, 'state', updatedState);
    
    // Also update other coagent properties that might have changed
    const currentCoagentState = context.coagentStatesRef.current[name];
    if (currentCoagentState) {
      Vue.set(reactiveCoAgent, 'nodeName', currentCoagentState.nodeName);
      Vue.set(reactiveCoAgent, 'threadId', currentCoagentState.threadId);
      Vue.set(reactiveCoAgent, 'running', currentCoagentState.running);
    }
  };

  // Now set the setState function reference in the reactive object
  reactiveCoAgent.setState = isExternalStateManagement(options) ? options.setState : setState;
  ...
  const unwatchThreadId = contextVueInstance.$watch(
    () => context.threadId,
    handleThreadChange,
    { immediate: true }
  );
  ...
  let unwatchExternalState: (() => void) | null = null;
  let unwatchInternalState: (() => void) | null = null;

  if (isExternalStateManagement(options)) {
    unwatchExternalState = contextVueInstance.$watch(
    ...
    unwatchInternalState = contextVueInstance.$watch(
  ...
  const unwatchConfig = contextVueInstance.$watch(
    () => {
      const newConfig = options.config
        ? options.config
        : options.configurable
  ...
  // Watch for coagent state changes and update reactive object
  const unwatchCoagentState = contextVueInstance.$watch(
    () => {
      ...
      return JSON.stringify(coagentState);
    },
    () => {
      const coagentState = getCoagentState({ 
        coagentStates: context.coagentStatesRef.current, 
      ...
      console.log('CoAgent watcher triggered, updating reactive object:', {
        name,
        coagentState,
        currentReactiveState: reactiveCoAgent.state
      });
      
      // Update reactive properties using Vue.set for proper reactivity
      Vue.set(reactiveCoAgent, 'name', name);
      Vue.set(reactiveCoAgent, 'nodeName', coagentState.nodeName);
      Vue.set(reactiveCoAgent, 'threadId', coagentState.threadId);
      Vue.set(reactiveCoAgent, 'running', coagentState.running);
      Vue.set(reactiveCoAgent, 'state', coagentState.state);
    },
    { immediate: true, deep: true }
  );

  // Enhanced cleanup function - called when component is destroyed
  const enhancedCleanup = () => {
    unwatchAvailableAgents();
    unwatchThreadId();
    unwatchConfig();
    unwatchCoagentState();
    if (unwatchExternalState) unwatchExternalState();
    if (unwatchInternalState) unwatchInternalState();
  };

  // Register cleanup on Vue instance destruction
  contextVueInstance.$once('hook:beforeDestroy', enhancedCleanup);

  return reactiveCoAgent;
}
...
export async function runAgent(
  ...
  if (hint) {
    ...
    if (hintMessage) {
      // For now, just log the hint message since we don't have direct access to appendMessage
      console.log('CoAgent hint message:', hintMessage);
      // TODO: Find a way to add this message to the chat
    } else {
      // We can't directly call runChatCompletion from here
      // This would need to be handled by the component using the CoAgent
      console.log('CoAgent: Triggering chat completion...');
    }
  } else {
    // We can't directly call runChatCompletion from here
    // This would need to be handled by the component using the CoAgent
